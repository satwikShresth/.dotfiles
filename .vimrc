...
let s:commonRgDict= {'window':{'width':0.9,'height':0.9},'source': 'rg --hidden --ignore-case --color=always --line-number --no-heading "."','sink': 'tabnew','options':"--ansi --color 'hl:-1:underline,hl+:-1:underline:reverse' --delimiter ':' --preview \"bat --color=always {1} --theme='Solarized (light)' --highlight-line {2}\""}
...
function! FuzzyRipAllCurrBuff()
  ...
  let l:localDict =copy(s:commonRgDict)
  let l:localDict['source']= "rg --hidden --ignore-case --color=always --line-number --no-heading \".\" ".join(l:keys," ")
